            ...
            try {
                if (Array.isArray(data)) {
                    ...
                    Object.keys(cronogramaPorInstructor).forEach((instructorId) => {
                        ...
                        instructor.clases.forEach((clase) => {
                            ...
                            fila += `<td class="hora-celda clase-asignada ${claseColor}" 
                                        ...
                                        data-telefono="${clase.estudiante_telefono}"
                    ...
                    $(document).on("click", ".clase-asignada", function () {
                        ...
                        // ✅ Ajuste: Eliminar los segundos de la hora
                        if (hora.includes(":")) {
                            let horasSeparadas = hora.split(" - ");
                            let horaInicio = horasSeparadas[0].substring(0, 5); // Extrae HH:MM
                            let horaFin = horasSeparadas[1].substring(0, 5);   // Extrae HH:MM
